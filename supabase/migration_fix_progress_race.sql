-- ============================================
-- MIGRATION: Race-safe challenge_progress-init
--
-- get_or_init_progress en de vier *_attempt-functies maakten de eerste
-- challenge_progress-rij aan met een check-dan-insert:
--   SELECT ...; IF NOT FOUND THEN INSERT ...; END IF;
-- Twee (bijna) gelijktijdige aanroepen voor hetzelfde team+challenge
-- zagen allebei "nog geen rij" en botsten op de unieke constraint:
--   duplicate key value violates unique constraint
--   "challenge_progress_team_id_challenge_id_key"
-- Een refresh loste het "op" omdat de herhaalde aanroep de rij van de
-- eerste, inmiddels voltooide poging gewoon vond.
--
-- Vervangen door één atomaire INSERT ... ON CONFLICT DO UPDATE, die de
-- bestaande rij teruggeeft bij een botsing in plaats van te falen. De
-- DO UPDATE is een no-op (zet team_id op zichzelf) — puur om RETURNING
-- te kunnen gebruiken op het conflict-pad; de bestaande state/started_at
-- van de rij blijft ongemoeid.
--
-- Verder byte-identiek aan de laatst gedeployde versie van elke functie
-- (dus scoring, placement, pogingenplafond, answer_matches en de
-- puzzle-attempts_remaining-seed blijven ongewijzigd).
--
-- Run in de Supabase SQL Editor, NA alle eerdere migraties.
-- ============================================

-- ── get_or_init_progress ──
...
BEGIN
  ...
  DO UPDATE SET team_id = challenge_progress.team_id
...
-- ── Grants (ongewijzigd, voor de zekerheid opnieuw) ──
